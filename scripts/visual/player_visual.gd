...
func _ready() -> void:
    ...
    if SpriteDb.frames("protagonist_custom") != null:
        ...
        # 2026-08-22 리마스터 v2: 1차본이 "소년미가 강하고 머리가 크다"는 지적을 받아
        # pro 로 다시 생성(성인 체형·작은 머리·갓 챙에 눈이 가리는 분위기).
        # 프레임 143x144(구르기를 pro 로 다시 뽑으며 가로로 넓어짐), idle 내용 높이 120.
        ...
        sprite_scale = 0.64                     # 120 * 0.64 ≈ 77px
        foot_offset = -45.0                     # 16/0.64 - (142 - 144/2)
    else:
        sheet = "protagonist"
    super._ready()
