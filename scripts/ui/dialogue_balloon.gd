...
# 4차 피드백(같은 날): "어색하지 않은 선에서 거의 꽉 차도 될 것 같다" → 0.65 로 재상향.
# 720 기준 뷰포트에서 초상화 높이 468px, 상단 여백 72px(약 10%) — 화면을 거의 채우되
# 정수리가 화면 끝에 닿아 답답해 보이지 않을 만큼은 남긴다. 더 올리면(0.75 근접) 여백이
# 거의 사라져 어색해진다는 게 판단 기준.
const PORTRAIT_HEIGHT_RATIO := 0.65
...
# 가장 넓은 것 기준으로 잡아야 모든 표정이 제 높이를 채운다.
