...
##
## 길손이 원혼을 하나 해원(진혼)할 때마다, 제 기억이 한 조각씩 지워진다 — 진혼의 값이다.
## v2(STORY_BIBLE v2)의 아이러니: 그가 달래는 넋들은 사랑하는 이의 집안에 서린 원한이고,
## **그녀를 지키려 싸울수록 그녀를 잊는다.** 짐을 덜어주는 일이 곧 자신을 지우는 일.
## 마지막엔 화면에 단 하나의 이름 '윤슬'(그녀의 아명)만 남고, 그마저 놓이면 글자가 빈다.
##
...
const FRAGMENTS := [
    {"id": "a_face",     "gut": 0, "label": "어느 집 문간에서 받아먹던 밥상 — 그 집 사람의 얼굴"},
    {"id": "her_voice",  "gut": 1, "label": "그이의 목소리 — 방금 강가에서 들은 그 목소리"},
    {"id": "mother",     "gut": 2, "label": "어머니의 얼굴 — 저도 무당이었고, 저도 이렇게 닳아 죽었다"},
    {"id": "true_name",  "gut": 3, "label": "제 본명 — 이후로는 '길손'이라는 호칭만 남는다"},
    {"id": "the_vow",    "gut": 4, "label": "그날 마당에서 끝내 삼킨 말, 둘이 주고받은 첫 약속"},
    {"id": "almost_all", "gut": 5, "label": "거의 전부 — 이제 화면엔 '윤슬' 한 이름만"},
    {"id": "yunseul",    "gut": 6, "label": "윤슬 — 사랑하는 이의 아명, 마지막 한 이름"},
]
