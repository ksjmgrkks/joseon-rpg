...
- **A 금줄 경계석 채택·적용 완료.** 아래 B/C/D는 향후 다른 스테이지에서 재검토할 수 있는 보존 후보다.
- 기존 `gate_barrier.png`의 빛기둥·세로 부적줄과 `level_exit_gate.png`의 일반 한옥문은 삭제했다.
- 각 시트는 왼쪽부터 `봉인 → 해제 중 → 통과 가능` 상태다.
- A안은 투명 배경 실게임 규격 6프레임으로 다시 만들었고 `GateArt`가 봉인·해제·열림을 공유한다.
...
1. 현재 전투 체인의 공통 통과 경계는 A 금줄 경계석으로 통일한다.
2. 봉인과 출구를 별도 그림으로 페이드 교체하지 않고, 하나의 구조가 실제로 해제된다.
3. 최종 시트는 Nearest 필터, 제한 팔레트, 투명 배경, 바닥 기준점 고정이다.
4. 런타임 자산은 `assets/sprites/fx/gate_geumjul_release.png`(192×208, 6프레임), 구현은 `scripts/world/gate_art.gd`와 `scripts/world/combat_gate.gd`다.
