extends SceneTree
##
## PixelLab 주인공 v2(보라 창, 224px) 애니 프레임 → protagonist_custom/<anim>.png 교체.
## 사용자 요청(2026-08-19): 크기/퀄리티 개선 + 창을 화려한 보라색으로.
##
## 실행: godot --headless --path . --script res://tools/pixel/assemble_protagonist_v2.gd -- --anim=idle --frames=4 --size=224
##
...
func _init() -> void:
    ...
    var size := int(args.get("size", "224"))
    if anim.is_empty() or frame_count <= 0:
        push_error("usage: --anim=<name> --frames=<n> [--size=224]")
        quit(1)
        return
