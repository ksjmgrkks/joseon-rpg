extends SceneTree
##
## PixelLab 「그슨대 노괴」 정체(거대 그림자) 프레임 → Godot 스프라이트 시트 조립.
##
## 실행: godot --headless --path . --script res://tools/pixel/assemble_elder.gd
##
## 입력:  .pl_tmp/elder/<anim>/<i>.png   (east 방향 프레임, PixelLab CDN 에서 받은 것)
## 출력:  assets/sprites/enemies/geuseondae_elder_shadow/<anim>.png (가로 스트립) + manifest.json
##
...
const SRC := "res://.pl_tmp/elder"
const OUT := "res://assets/sprites/enemies/geuseondae_elder_shadow"
const ANIMS := {
    "idle":      {"frames": 7, "fps": 6,  "loop": true},
