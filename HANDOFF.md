...
## 현재 상태 (한 줄 요약)
> **지금 여기:** **처음부터 끝까지 완주 가능한 게임 (2026-06-12, 2차).** 타이틀 → **오프닝 프롤로그(호환 괴담 도입 5문단)** → 마을(어르신 퀘스트 수락) → 들판(약초·서찰 픽업) → 숲(적 5종) → 보스 아레나(두령 일갈→전투) → 마을 귀환(어금니 보고→어르신 고백) → 엔딩(사이드 완료수별 3분기) 까지 한 줄기로 연결. **엔드투엔드 통합 테스트(test_playthrough)로 메인 퀘스트 5단계 완주·부적→호신부 해금·엔딩 분기를 검증.** 이어하기는 저장 지역으로 복귀, 보스 귀환 동선 정합, 픽업은 아이콘+부유 연출. 헤드리스 테스트 15종 그린. 전투 밸런스(평타 ~7초로 보스 처치, 회피·호신부 여유) 수치 확인.
