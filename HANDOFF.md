...
## 현재 상태 (한 줄 요약)
> **지금 여기:** **서사 설계 19차 — 「해원(解冤)」 스토리 코어 확정 (2026-06-28).** 사용자가 *산나비식 비장미*를 원함 → 정서 코어 **⑤ 망각 + ⑥ 진혼** 융합 채택, '산'·기존 몬스터·"내가 괴물" 집착 모두 내려놓음. **확정:** 역할/이름 **'해원'**(원혼을 달래는 자), 당위 **ⓑ 기근의 선택 + ⓔ 사랑하는 이**(고을 문을 닫아 모두도·제 딸 여울도 잃은 현감). **시그니처 둘:** ① 전투=진혼(베는 게 아니라 달래는 것, 후반 의미 전복 — 15차 '혼 흩어짐' VFX가 이미 톤 일치) ② 진혼할 때마다 대사·HUD·이름 글자가 지워짐(끝엔 '여울' 한 이름만 남고 그마저 소거=자기 천도). **전문 → `docs/STORY_BIBLE.md`**(주인공 설정·6굽이 비트시트·대사 샘플 전량). **★남은 결정 1개:** 무대 그릇 — 추천 '물등(Ⓒ)+동틀녘 굿(Ⓓ)+빈집(Ⓐ)' 융합 vs 단순화. 그릇 정해지면 스테이지 JSON 스켈레톤 + '기억 지워짐' 시스템 프로토타입으로.
...
> 20. **[현재] 「해원」 서사 설계 — 코어 확정, 무대 그릇 결정 대기.** 코어/주인공/비트시트는 `docs/STORY_BIBLE.md`에 잠금(이름='해원', 당위=ⓑ기근+ⓔ딸 여울, 시그니처=진혼+망각). **다음 행동:** ① 무대 그릇 확정(추천 물등+동틀녘+빈집 / 또는 단순화) → ② 6굽이 스테이지 JSON 스켈레톤(기존 `stage.gd` 빌더 재활용, `GAMEPLAY_ONLY=false`로 스토리 모드 복원하되 데이터는 새 바이블 기준) → ③ '기억 지워짐'(글자 페이드·소거) 시스템 프로토타입 → ④ '걷는 회상' 1장면(수묵 흑백 오버랩, 조작 유지) → ⑤ 1막 대사 데이터 + 수묵 한 폭 컷 1장.
