...
- **2026-06-29 (23차 — 시그니처 '기억 지워짐' 시스템 + 6굽이 스테이지/대사 데이터)** — 사용자 텔레그램 "조선 RPG 쭉 이어서 진행 + 필요하면 PixelLab 적극 활용". 바이블 '다음 제작 순서' 2·3단계 실제 구현. ① `scripts/memory/memory_ledger.gd`(오토로드 `MemoryLedger`) — 굽이별 기억조각 7개(FRAGMENTS), `erase`/`erase_for_gut`/`progress`/`is_erased`/`name_intact`/`screen_names`/`next_fragment`, 신호 `memory_erased`·`ledger_changed`·`last_name_released`, SaveManager `"memory"` 키 save/load. ② `scripts/memory/memory_glyph.gd`(class_name `MemoryGlyph`, RefCounted) — 순수 결정적 글자 dissolve(BBCode)·strip(공백)·erased_indices, seed별 패턴·비율 단조증가. ③ `project.godot` autoload 에 MemoryLedger 등록(Flags 다음). ④ 스테이지 7장 `assets/stages/haewon_{0_prologue,1_ferry,2_market,3_village,4_watergate,5_emptytown,6_yunseul}.json` — 비트시트·물/밤/동틀녘 backdrop·기존 적 재활용·전진게이트·환경서사 픽업·강따라 체인 exits(새 .tscn 래퍼는 다음 단계). ⑤ 대사 14편 `assets/dialogue/haewon/*.json`(프롤로그 open/after, ferry_omen, ferryman_spirit, recall_yunseul[윤슬 회상], market_flooded, village_drowned/confession, watergate_approach/reveal, emptytown_enter, empty_house, yunseul_approach/release) — 바이블 사극체 그대로. ⑥ `tests/test_memory_ledger.{gd,tscn}` 8케이스(ledger 5 + glyph 3). **검증: WSL Godot 없음 → JSON 21장 파이썬 유효성 통과, GDScript는 정적 점검만(autoload 파싱 안정 위해 const 타입주석 제거·typed 리턴 보수화). 헤드리스/인게임은 PC 대기.** 미배선(다음): GAMEPLAY_ONLY=false 복원 + 굽이클리어→erase_for_gut 연결 + .tscn 래퍼 + 글자 dissolve UI 적용 + 걷는 회상 + 물등 PixelLab.
