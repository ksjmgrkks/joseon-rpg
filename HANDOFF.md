...
## 현재 상태 (한 줄 요약)
> **지금 여기:** **서사 설계 20차 — 「해원(解冤)」 스토리 코어 + '여울=사랑하는 이' 확정 (2026-06-28).** 19차 코어 위에, 사용자 지시로 **마지막 한 사람 '여울'을 딸 → 사랑하는 이(정인)로 변경**. 이제 당위 ⓔ는 "義/생존" 위에 **사랑 vs 직분**까지 겹친다(닫힌 문 밖에 정인 여울이 있었으나 직분을 택해 닫음). 5굽이 빈집 단서도 아이 소품(댕기·짚신)→**정인 소품(비녀·꽃신·가락지)**으로 교체. 또 텔레그램 비트 명문화: 그가 달래는 원혼=그 당위로 죽은 이들, 해원할 때마다 **죄책이 지워짐=속죄+망각+자기소멸**, 반전은 단죄 아닌 **용서·놓아줌**. (`docs/STORY_BIBLE.md` 반영 완료.) **확정:** 역할/이름 **'해원'**, 당위 **ⓑ 기근의 선택 + ⓔ 사랑하는 이**(고을 문을 닫아 모두도·정인 여울도 잃은 현감). **시그니처 둘:** ① 전투=진혼(베는 게 아니라 달래는 것, 후반 의미 전복 — 15차 '혼 흩어짐' VFX가 이미 톤 일치) ② 진혼할 때마다 대사·HUD·이름 글자가 지워짐(끝엔 '여울' 한 이름만 남고 그마저 소거=자기 천도). **★남은 결정 1개:** 무대 그릇 — 추천 '물등(Ⓒ)+동틀녘 굿(Ⓓ)+빈집(Ⓐ)' 융합 vs 단순화. 그릇 정해지면 스테이지 JSON 스켈레톤 + '기억 지워짐' 시스템 프로토타입으로.
...
> 20. **[현재] 「해원」 서사 설계 — 코어 확정, 무대 그릇 결정 대기.** 코어/주인공/비트시트는 `docs/STORY_BIBLE.md`에 잠금(이름='해원', 당위=ⓑ기근+ⓔ사랑하는 이 여울, 시그니처=진혼+망각). **다음 행동:** ① 무대 그릇 확정(추천 물등+동틀녘+빈집 / 또는 단순화) → ② 6굽이 스테이지 JSON 스켈레톤(기존 `stage.gd` 빌더 재활용, `GAMEPLAY_ONLY=false`로 스토리 모드 복원하되 데이터는 새 바이블 기준) → ③ '기억 지워짐'(글자 페이드·소거) 시스템 프로토타입 → ④ '걷는 회상' 1장면(수묵 흑백 오버랩, 조작 유지) → ⑤ 1막 대사 데이터 + 수묵 한 폭 컷 1장.
