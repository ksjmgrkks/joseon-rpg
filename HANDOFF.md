...
> 20. **[현재] 「해원」 서사 설계 — 코어 확정, 무대 그릇 결정 대기.** 코어/주인공/비트시트는 `docs/STORY_BIBLE.md`에 잠금(이름='해원', 당위=ⓑ수문(홍수)+ⓔ정인 윤슬, 시그니처=진혼+망각). **다음 행동:** ① 무대 그릇 확정(추천 물등+동틀녘+빈집 / 또는 단순화) → ② 6굽이 스테이지 JSON 스켈레톤(기존 `stage.gd` 빌더 재활용, `GAMEPLAY_ONLY=false`로 스토리 모드 복원하되 데이터는 새 바이블 기준) → ③ '기억 지워짐'(글자 페이드·소거) 시스템 프로토타입 → ④ '걷는 회상' 1장면(수묵 흑백 오버랩, 조작 유지) → ⑤ 1막 대사 데이터 + 수묵 한 폭 컷 1장.
